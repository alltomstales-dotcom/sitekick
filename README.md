...
1. **Residency map** — **Logical** (default SVG/HTML canvas) or **Geo** (MapLibre + Carto Dark Matter GL / raster OSM fallback, no API keys) toggle. Western/central PA SIM-approx coords for Highmark HQ, AHN flagships, and external hubs; markers colored by family/external/payer. Filters, node detail drawer, and **click-path narratives** (dim non-path, brighten path + geo line highlight; clear with Esc / Clear path).
...
- lucide-react icons
- maplibre-gl + Carto Dark Matter GL (raster OSM fallback; no API keys)
